      ...
      <div style={{ display: 'flex', padding: '28px 40px 40px 40px' }}>
        ...
        <div style={{ width: '32%', paddingRight: 28 }}>
          ...
          {data.portfolio_images && data.portfolio_images.length > 0 && (() => {
            const imgs = data.portfolio_images.slice(0, 4)
            const count = imgs.length
            const ROW_H = 72
            const GAP = 5
            const totalH = count <= 2 ? ROW_H : ROW_H * 2 + GAP
            return (
              <div style={{ marginBottom: 28 }}>
                <div style={{ marginBottom: 14 }}>
                ...
                <div style={{ display: 'grid', gridTemplateColumns: count === 1 ? '1fr' : '1fr 1fr', gap: GAP, height: totalH }}>
                  {imgs.map((img: string, i: number) => (
                    <img key={i} src={img} crossOrigin="anonymous"
                      style={{ width: '100%', height: '100%', objectFit: 'cover' as const, borderRadius: 4, display: 'block',
                        ...(count === 3 && i === 2 ? { gridColumn: 'span 2' } : {}) }}
                      alt="Project" />
                  ))}
                </div>
              </div>
